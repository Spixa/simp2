...
private:
  ...
  void write_header() {
    asio::async_write(socket_,
                      asio::buffer(&message_out_queue_.front().header,
  ...
  void write_body() {
    asio::async_write(socket_,
                      asio::buffer(message_out_queue_.front().body.data(),
                                   message_out_queue_.front().body.size()),
                      [this](std::error_code ec, std::size_t length) {
                        if (!ec) {
                          message_out_queue_.pop_front();
...
private: // reading
  void manage_read() { read_header(); }
  /* async */
  void read_header() {
    asio::async_read(
        socket_,
        asio::buffer(&temporary_in_message.header, sizeof(message_header)),
        [this](std::error_code ec, std::size_t length) {
          if (!ec) {
            if (temporary_in_message.header.size > 0) {
              temporary_in_message.body.resize(
                  temporary_in_message.header.size);
              read_body();
            } else {
  ...
  void read_body() {
    asio::async_read(socket_,
                     asio::buffer(temporary_in_message.body.data(),
                                  temporary_in_message.body.size()),
                     [this](std::error_code ec, std::size_t length) {
                       if (!ec) {
                         add_to_incoming_message_queue();
                       } else {
  ...
  virtual void add_to_incoming_message_queue() {
    if (owner_type_ == owner::server) {
      message_in_queue_.push_back(
          {this->shared_from_this(), temporary_in_message});
    } else {
